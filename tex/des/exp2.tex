...
\subsection{Consumo energetico en relacion al SITI}\label{section:exp2.2-sitixconsumo}
% FASE 2.2 
 % - Videos para pruebas:
...
\subsection{Relación entre numero de cores, tiempo de codificación y consumo energetico}\label{section:exp2.3-coresxconsumo}
    

En esta segunda subfase de experimentación, perteneciente a la sección \textbf{Consumo energético en relación al SITI y el número de Threads} \ref{exp2-desarrollo}, se ha analizado el consumo energético de procesos de transcodificación ejecutados sobre contenedores de Docker con limitaciones en el acceso a los cores de la CPU y en el número de threads utilizados por los codificadores. Bajo esta premisa, se han desarrollado los experimentos descritos a continuación, con el objetivo de estudiar la relación existente entre el consumo energético, el tiempo de codificación y el número de cores y threads disponibles durante el proceso.

Para esta subfase experimental se ha utilizado el conjunto de \textit{vídeos} descrito en \ref{tab:exp2.2-videos}, creado y clasificado durante la subfase \textbf{Consumo energético en relación al SITI} \ref{section:exp2.2-sitixconsumo}. Del conjunto de vídeos compuesto por un único \textit{chunk}, se han seleccionado dos vídeos correspondientes a cada una de las cuatro zonas definidas en \ref{tab:siti-zones-definition}, obteniendo un total de \textbf{ocho vídeos}, que se muestran en la Tabla \ref{tab:exp2.3-videos-30s}.
...
\begin{itemize}
 ...
 \item \textbf{plot-tiempoxcores:} Genera graficas lineales las cuales se agrupan por nombre del video, bitrate y por generacion de codificador (\textit{x264 frente a h264} o \textit{265 frente a hevc}). Estas graficas comparan el tiempo de codificacion (Eje Y) con el numero de coners empleado en dico proceso (Eje X). El objetivo es compara el tiempo necesitado en cada proceso de codificacion respecto al numero de cores y threads que se han utilizado. 

 \item \textbf{plot-consumoxtiempo:} Genera un conjunto de graficas de puntos agrupadas por nombre del video, generacion de codificacodr (\textit{x264 frente a h264} o \textit{265 frente a hevc}) y por bitrate, las cuales comparan el consumo energetico(Eje X) frente el tiempo que ha tardado en codificarlo (Eje Y). En cada una de las graficas que se observa dos grupos de puntos, los cuales pertenecen a un codificador basado CPU o un codificador basado en GPU. Cada punto se encuentra numerado en base al numero de cores empleado en su codificacion. El objteivo de estas graficas es compara la eficiencia en tiempo y consumo energetico en relacion con el numero de cores.
