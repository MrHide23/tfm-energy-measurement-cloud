...
\subsection{Post-procesamiendo de las Métricas de Consumo Energético}
 ...
 \begin{itemize}
     ...
     \item \texttt{Container Energy Monitoring} \ref{fig:exp1-monitor-csv_header}
     \end{itemize}
...
\begin{itemize} 
 \item \texttt{em\_total\_energy}: Energia total del sistema estimada por Global Energy Meter en base a las metricas recolectadas de RAPL
...
\subsubsection{Escenario 1: Transcodificacion sobre un contenedor empleando CPU y GPU}
 ...
 En cuanto a la energia que el proceso de transcodificacion ha consumido, con Container Energy Monitoring y con Scaphandre se observa que existe una diferencia entre el consumo energético del nodo(\textbf{node\_total\_joules}) y el consumo del proceso(\textbf{node\_total\_joules}) llevado a cabo en el contenedor. Esta diferencia ronda los \textbf{100-200 Julios}, que supone entre un \textbf{\(5.8\%\)} y \textbf{\(8.9\%\)} respecto al total de la energia consumida por el nodo. Este hecho puede deberse a la existencia de ruido energetico posiblemente deribado de aquellos componentes hardward sobre los que RAPL no puede tomar metricas. De ahora en adelante nos referiremos a esta energia no medible como \textit{ruido del sistema $E_{ruido\_sistema}$}.
 
 Cabe destacar, que en el caso de las metricas de Kepler, el consumo energético del proceso presenta muy poca diferencia respecto al consumo energetico del nodo, siendo la diferencia energetica mucho mayor respecto a la energia medida por Global Energy Meter (\textbf{em\_total\_energy}). Este hecho puede deberse a que hipeteticamente Kepler este calculando la energia total del nodo en base a la energia del proceso de la siguiente forma \(E_{energi\_nodo}\approx E_{ruido\_sistema}+\sum_{0}^{n} E_{proceso\_n}\).
...
\subsubsection{Escenario 2: Transcodificacion empleando un contenedor unicamente con CPU}
 ...
 En relación a las métricas de consumo del nodo, se aprecia nuevamente que Container Energy Monitoring tiene una menor diferencia respecto a al consumo energético del nodo frente al consumo energético medido por Global Energy Meter. Al igual que en el anterior escenario, se observa que existe una diferencia entre el consumo energético del proceso de codificación y el consumo energético total del nodo, y que de la misma forma el consumo energético del proceso es anormalmente bajo. Unos resultados muy similares a los del escenario 1, sin el consumo energético de la GPU.
...
\begin{table}[h!]
 ...
 Herramienta                        & E.N(\%)\tablefootnote{EN: Error de la energia del nodo respecto a la energia total del sistema obtenida con Global Energy Meter}                             & E.N(\%)                            & \multicolumn{1}{c|}{\cellcolor[HTML]{C0C0C0}E.N(\%)}          & EP(\%)          & \multicolumn{1}{c|}{\cellcolor[HTML]{C0C0C0}E.N(\%)}          & EP(\%)\tablefootnote{EP: Energia del proceso respecto a la energia del nodo calculada por la herramienta}          \\ \hline
 ...
 \cellcolor[HTML]{EFEFEF}Container Energy Monitoring    & 0.4                                 & 0.31                               & \multicolumn{1}{c|}{0.25}                                     & 82.10           & \multicolumn{1}{c|}{7.23}                                     & 92,77           \\ \hline
 \end{tabular}
